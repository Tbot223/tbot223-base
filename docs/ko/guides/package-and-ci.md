...
- mypy용 optional `type` dependency group.
- build와 metadata validation tool을 위한 optional `release` dependency group.
- test, type, release tool을 함께 설치하는 optional `dev` dependency group.
- `tbot223_base` package discovery.
- type-aware tooling을 위한 `py.typed` 포함.
- pytest 기본 `tests` path.
- mypy package/public consumer-check target과 Python 3.10 typing baseline.
...
이 script는 metadata check, Python compile check, `pytest`, mypy, `actionlint`, `git diff --check`, temporary source/wheel build, `twine check`, distribution metadata assertion, isolated installed-wheel smoke check를 실행한다.
...
각 job은 package와 test/type dependency를 설치한 뒤 다음을 실행한다.
...
- built wheel을 격리 환경에 설치하고 import/public payload smoke check를 통과한다.
